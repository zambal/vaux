-module(vaux_htmerl_sax_utf8).

%% Modified version of https://github.com/zadean/htmerl
...
-define(ascii_hex_digit(L),
        ?ascii_digit(L) orelse L >= $A andalso L =< $F orelse L >= $a andalso L =< $f).
...
string(Bin, Options) ->
    ...
    M3 = case proplists:get_value(fragment_mode, Options) of
             undefined ->
                 M2;
             B when is_boolean(B) ->
                 M2#{fragment_mode => B}
...
default_state() ->
    ...
    #{%% initial
      ...
      %%  | in_cell | in_select | in_select_in_table | in_component
...
rcdata(<<>>, State) ->
    emit(eof, State);
rcdata(Stream, #{line_num := LineNum, nl_cp := NlCp} = State) ->
...
plaintext(<<>>, State) ->
    emit(eof, State);
plaintext(Stream, #{line_num := LineNum, nl_cp := NlCp} = State) ->
    {Len, LineNum1} = find_stop(Stream, [<<0>>], LineNum, NlCp),
    <<Chars:Len/binary, Rest/binary>> = Stream,
    State1 = emit(#chars{data = Chars}, State),
...
rcdata_end_tag_name(<<$>, Rest/binary>>,
    ...
    data(Rest, State1);
rcdata_end_tag_name(<<C, Rest/binary>>,
                    #{current_token := Curr, temp_buffer := Buff} = State)
...
rcdata_end_tag_name(Stream, #{temp_buffer := Buff} = State) ->
    State1 = emit(#char{data = $<}, State),
    State2 = emit(#char{data = $/}, State1),
    F = fun(Ch, S) -> emit(#char{data = Ch}, S) end,
...
rawtext_end_tag_name(Stream, #{temp_buffer := Buff} = State) ->
    State1 = emit(#char{data = $<}, State),
    State2 = emit(#char{data = $/}, State1),
    F = fun(Cf, S) -> emit(#char{data = Cf}, S) end,
    State3 = lists:foldr(F, State2, Buff),
...
script_data_escaped_less_than_sign(<<C, _/binary>> = Stream, State)
    when ?ascii_letter(C) ->
    State1 = emit(#char{data = $<}, State),
...
before_attribute_name(<<C, _/binary>> = Stream, State0) when C == $/; C == $> ->
...
before_attribute_name(Stream, #{current_token := Curr} = State0) ->
    ...
    attribute_name(Stream,
                   State0#{current_token := Curr#start_tag{attributes = [Tok | Atts]}}).

%% 8.2.4.33
attribute_name(<<>>, State) ->
    after_attribute_name(<<>>, State);
attribute_name(<<C, _/binary>> = Stream, State) when ?ws(C); C == $/; C == $> ->
    after_attribute_name(Stream, State);
attribute_name(<<$=, Rest/binary>>, State) ->
    before_attribute_value(Rest, State);
attribute_name(Stream,
               #{current_token := #start_tag{attributes = [At | Ats]} = Curr} = State) ->
...
attribute_name_1(<<C, _/binary>> = Stream, Acc) when ?ws(C); C == $/; C == $>; C == $= ->
    {Acc, Stream};
...
attribute_value_double_quoted(<<>>, State) ->
    % fatal parse error
    emit(eof, State);
attribute_value_double_quoted(Stream,
                              #{current_token := Curr,
                                att_dq_stop := Stop,
                                nl_cp := NlCp,
                                line_num := LineNum} =
...
attribute_value_single_quoted(<<>>, State) ->
    % fatal parse error
    emit(eof, State);
attribute_value_single_quoted(Stream,
                              #{current_token := Curr,
                                att_sq_stop := Stop,
                                nl_cp := NlCp,
                                line_num := LineNum} =
...
markup_declaration_open(<<D, O, C, T, Y, P, E, Rest/binary>>, State)
    when D =:= $d orelse D =:= $D, O =:= $o orelse O =:= $O, C =:= $c orelse C =:= $C,
         T =:= $t orelse T =:= $T, Y =:= $y orelse Y =:= $Y, P =:= $p orelse P =:= $P,
         E =:= $e orelse E =:= $E ->
    doctype(Rest, State);
...
comment(Stream,
    ...
    comment(Rest, State#{current_token := Tok, line_num := LineNum1}).
...
after_doctype_name(<<P, U, B, L, I, C, Rest/binary>>, State)
    when P =:= $p orelse P =:= $P, U =:= $u orelse U =:= $U, B =:= $b orelse B =:= $B,
         L =:= $l orelse L =:= $L, I =:= $i orelse I =:= $I, C =:= $c orelse C =:= $C ->
    after_doctype_public_keyword(Rest, State);
after_doctype_name(<<S, Y, Z, T, E, M, Rest/binary>>, State)
    when S =:= $s orelse S =:= $S, Y =:= $y orelse Y =:= $Y, Z =:= $s orelse Z =:= $S,
         T =:= $t orelse T =:= $T, E =:= $e orelse E =:= $E, M =:= $m orelse M =:= $M ->
...
before_doctype_public_identifier(<<C, Rest/binary>>, State) when ?ws(C) ->
    before_doctype_public_identifier(Rest, State);
before_doctype_public_identifier(<<$\", Rest/binary>>,
                                 #{current_token := Curr} = State) ->
    Tok = Curr#doctype{public = <<>>},
    doctype_public_identifier_double_quoted(Rest, State#{current_token := Tok});
...
doctype_public_identifier_double_quoted(<<0, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    % parse error
    Tok = append_to_public(16#FFFD, Curr),
    doctype_public_identifier_double_quoted(Rest, State#{current_token := Tok});
doctype_public_identifier_double_quoted(<<$>, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    % parse error
    Tok = Curr#doctype{quirks = true},
    State1 = emit(Tok, State),
...
doctype_public_identifier_double_quoted(<<C/utf8, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
...
doctype_public_identifier_single_quoted(<<$', Rest/binary>>, State) ->
    after_doctype_public_identifier(Rest, State);
doctype_public_identifier_single_quoted(<<0, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    % parse error
    Tok = append_to_public(16#FFFD, Curr),
    doctype_public_identifier_single_quoted(Rest, State#{current_token := Tok});
doctype_public_identifier_single_quoted(<<$>, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    % parse error
    Tok = Curr#doctype{quirks = true},
    State1 = emit(Tok, State),
...
doctype_public_identifier_single_quoted(<<C/utf8, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    Tok = append_to_public(C, Curr),
    doctype_public_identifier_single_quoted(Rest, State#{current_token := Tok}).
...
between_doctype_public_and_system_identifiers(<<C, Rest/binary>>, State) when ?ws(C) ->
    between_doctype_public_and_system_identifiers(Rest, State);
between_doctype_public_and_system_identifiers(<<$>, Rest/binary>>,
                                              #{current_token := Curr} = State) ->
    State1 = emit(Curr, State),
    data(Rest, State1);
between_doctype_public_and_system_identifiers(<<$\", Rest/binary>>,
...
between_doctype_public_and_system_identifiers(<<>>, #{current_token := Curr} = State) ->
    ...
    State1 = emit(Tok, State),
    emit(eof, State1);
between_doctype_public_and_system_identifiers(<<_, Rest/binary>>,
                                              #{current_token := Curr} = State) ->
    % parse error
    Tok = Curr#doctype{quirks = true},
    bogus_doctype(Rest, State#{current_token := Tok}).
...
before_doctype_system_identifier(<<C, Rest/binary>>, State) when ?ws(C) ->
    before_doctype_system_identifier(Rest, State);
before_doctype_system_identifier(<<$\", Rest/binary>>,
                                 #{current_token := Curr} = State) ->
    Tok = Curr#doctype{system = <<>>},
    doctype_system_identifier_double_quoted(Rest, State#{current_token := Tok});
...
doctype_system_identifier_double_quoted(<<0, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    % parse error
    Tok = append_to_system(16#FFFD, Curr),
    doctype_system_identifier_double_quoted(Rest, State#{current_token := Tok});
doctype_system_identifier_double_quoted(<<$>, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    % parse error
    Tok = Curr#doctype{quirks = true},
    State1 = emit(Tok, State),
...
doctype_system_identifier_double_quoted(<<C/utf8, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
...
doctype_system_identifier_single_quoted(<<$', Rest/binary>>, State) ->
    after_doctype_system_identifier(Rest, State);
doctype_system_identifier_single_quoted(<<0, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    % parse error
    Tok = append_to_system(16#FFFD, Curr),
    doctype_system_identifier_single_quoted(Rest, State#{current_token := Tok});
doctype_system_identifier_single_quoted(<<$>, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    % parse error
    Tok = Curr#doctype{quirks = true},
    State1 = emit(Tok, State),
...
doctype_system_identifier_single_quoted(<<C/utf8, Rest/binary>>,
                                        #{current_token := Curr} = State) ->
    Tok = append_to_system(C, Curr),
    doctype_system_identifier_single_quoted(Rest, State#{current_token := Tok}).
...
cdata_section(Stream, #{line_num := LineNum, nl_cp := NlCp} = State) ->
...
character_reference(<<C, _/binary>> = Stream, State) when ?ws(C); C =:= $<; C =:= $& ->
...
character_reference(Stream, State) ->
    case vaux_htmerl_util:entity(Stream) of
        {[], _, _} ->
...
numeric_character_reference(<<C, Rest/binary>>, #{temp_buffer := Buff} = State0)
    when C =:= $x; C =:= $X ->
    hexadecimal_character_reference_start(Rest,
                                          State0#{temp_buffer := [C | Buff], char_ref_code := 0});
...
hexadecimal_character_reference(<<C1, C2, C3, $;, Rest/binary>>, State0)
    when ?ascii_hex_digit(C1), ?ascii_hex_digit(C2), ?ascii_hex_digit(C3) ->
...
%% 8.2.4.77
decimal_character_reference(<<C1, $;, Rest/binary>>, State0) when ?ascii_digit(C1) ->
...
decimal_character_reference(<<C1, C2, C3, $;, Rest/binary>>, State0)
    when ?ascii_digit(C1), ?ascii_digit(C2), ?ascii_digit(C3) ->
    Code1 = list_to_integer([C1, C2, C3]),
    numeric_character_reference_end(Rest, State0#{char_ref_code := Code1});
decimal_character_reference(<<C1, C2, C3, C4, $;, Rest/binary>>, State0)
    when ?ascii_digit(C1), ?ascii_digit(C2), ?ascii_digit(C3), ?ascii_digit(C4) ->
...
decimal_character_reference(<<C1, C2, C3, C4, C5, C6, C7, $;, Rest/binary>>, State0)
    when ?ascii_digit(C1), ?ascii_digit(C2), ?ascii_digit(C3), ?ascii_digit(C4),
         ?ascii_digit(C5), ?ascii_digit(C6), ?ascii_digit(C7) ->
...
numeric_character_reference_end(Stream, #{char_ref_code := Code} = State0) ->
    Code1 = vaux_htmerl_util:mask_code_ref(Code),
    character_reference_end(Stream, State0#{temp_buffer := [Code1]}).

%% 8.2.4.79
character_reference_end(Stream,
                        #{temp_buffer := Buff,
                          current_token := Curr,
                          return_fun := Return} =
                            State) ->
    State1 =
        if Return == attribute_value_double_quoted;
           Return == attribute_value_single_quoted;
           Return == attribute_value_unquoted ->
               Tok = lists:foldr(fun(C, A) -> append_to_att_value(C, A) end, Curr, Buff),
               State#{current_token := Tok};
           true ->
               lists:foldr(fun(C, A) -> emit(#char{data = C}, A) end, State, Buff)
        end,
    case Return of
        data ->
            data(Stream, State1);
...
%% before_html state
dispatch(#{insertion_mode := before_html, fragment_mode := FMode} = State, Token) ->
    case Token of
        #doctype{} ->
            % parse error
            State;
...
%% before_head state
dispatch(#{insertion_mode := before_head, fragment_mode := FMode} = State, Token) ->
    case Token of
        #char{data = C} when ?ws(C) ->
...
dispatch(#{insertion_mode := in_head_noscript} = State, _Token) ->
    State;
%% after_head state
dispatch(#{insertion_mode := after_head, fragment_mode := FMode} = State, Token) ->
    case Token of
        #char{data = C} when ?ws(C) ->
            add_text_char(C, State);
        #comment{data = Comment} ->
...
dispatch(#{insertion_mode := in_body} = State, Token) ->
    case Token of
        ...
        #start_tag{name = Name}
            ...
            State1 = maybe_pop_text(State),
            State2 = add_html_element(Token, State1),
            State2#{insertion_mode := text, orig_insertion_mode := in_body};
        #end_tag{name = Name}
            when Name == <<"title">>;
                 Name == <<"noscript">>;
                 Name == <<"noframes">>;
        ...
        #start_tag{name = Name} when Name == <<"li">>; Name == <<"dd">>; Name == <<"dt">> ->
            State1 = maybe_pop_text(State),
            State2 =
                case get_current_node(State1) of
                    % close
        ...
        #end_tag{name = <<"br">>} ->
            add_html_element(#start_tag{name = <<"br">>, closing = true}, State);
    ...
    case Token of
        #char{}
            when Curr == <<"table">>;
                 ...
                 Curr == <<"thead">>;
                 Curr == <<"tr">> ->
            dispatch(State#{insertion_mode := in_table_text, orig_insertion_mode := in_table},
                     Token);
        #expr{}
            when Curr == <<"table">>;
                 Curr == <<"tbody">>;
...
dispatch(#{insertion_mode := in_table_body} = State, Token) ->
    case Token of
        ...
        #start_tag{name = N}
            ...
            case get_current_node(State) of
                #start_tag{name = Curr}
                    when Curr == <<"tbody">>; Curr == <<"thead">>; Curr == <<"tfoot">> ->
                    State1 = maybe_pop_text(State),
...
dispatch(#{insertion_mode := in_select} = State, Token) ->
    case Token of
        ...
        #start_tag{name = N} when N == <<"input">>; N == <<"textarea">> ->
            % parse error
            case is_open(<<"select">>, State) of
                false ->
                    State;
...
%% in_component state
dispatch(#{insertion_mode := in_component} = State, Token) ->
    case Token of
        #char{} ->
            State1 = dispatch(State#{insertion_mode := in_body}, Token),
        ...
        #start_tag{name = N} when N == <<"td">>; N == <<"th">> ->
...
%% add_component_insertion_mode(Mode, #{component_ins_modes := Modes} = State) ->
%%    State#{component_ins_modes := [Mode|Modes]}.
%%
%% get_component_insertion_mode(#{component_ins_modes := [M|Modes]}) -> M;
%% get_component_insertion_mode(#{component_ins_modes := []}) -> undefined.
%%
%% pop_component_insertion_mode(#{component_ins_modes := [_|Ms]} = State) ->
%%    State#{component_ins_modes := Ms};
%% pop_component_insertion_mode(State) ->
%%    State.
...
adjust_att_name(Name) ->
    case vaux_htmerl_util:adjusted_attribute_name(Name) of
        {xlink, L} ->
            {?XLINK, <<"xlink">>, L};
        {xml, L} ->
            {?XML, <<"xml">>, L};
...
add_html_element(#start_tag{name = N,
    ...
    Atts1 =
        [begin
         ...
         || #attribute{name = AName, value = AValue} <- Atts, AName =/= <<"xmlns">>],
    U = u(N),
...
add_math_element(#start_tag{name = N,
    ...
    case Closing of
        false ->
            add_open_element(Token#start_tag{name = {mathml, N}},
                             State2#{inscope_namespace := [mathml | Ns]});
        true ->
...
add_svg_element(#start_tag{name = N,
    ...
    case Closing of
        false ->
            add_open_element(Token#start_tag{name = {svg, N}},
                             State2#{inscope_namespace := [svg | Ns]});
        true ->
